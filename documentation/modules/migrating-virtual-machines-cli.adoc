...
stringData:
  user: <user> <1>
  password: <password> <2>
  insecureSkipVerify: <true/false> <3>
  cacert: | <4>
...
<3> Specify the `true` or `false` in the `insecureSkipVerify` field. `true` skips the
<4> {rhv-short} only: Specify the CA certificate of the {manager}. You can retrieve it at `https://<engine_host>/ovirt-engine/services/pki-resource?resource=ca-certificate&format=X509-PEM-CA`.
